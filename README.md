...
- **A+ · Inferential-modeling layer** (`agent/modeling.py`): questions like *"what predicts 30-day readmission, adjusting for age and sex?"* route to a real **logistic / OLS / Cox (survival) / association** model — the agent builds a patient-level analytic dataset via SQL, fits the model with `statsmodels`, and returns **adjusted odds/hazard ratios with 95% CIs and p-values** — rendered as **forest plots**, with **Kaplan-Meier survival curves** for time-to-event outcomes. This is the covariate-adjusted model the guardrail keeps recommending — now the agent *does* it, not just flags the need.
